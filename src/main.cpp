...
// get open and close prices outside the main while loop
double get_open(const string& fn, const string& date) {
    ifstream infile {fn};
    ...
    while (getline(infile, holder)) {
        ...
        if (d == date) return stod(x);
    }
}

double get_close(const string& fn, const string& date) {
    ifstream infile {fn};
    ...
    while (getline(infile, holder)) {
        ...
        if (d == date) return stod(y);
    }
}

double get_avg(const string& fn, const string& date) {
    ifstream infile {fn};
    ...
    while (getline(infile, holder)) {
        ...
        if (d == date) return ( (stod(x) + stod(y)) / 2 );
    }
}
...
double weighted_days_below(const string& fn, const string& start, const string& end) {
    ifstream infile {fn};
